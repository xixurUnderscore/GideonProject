...
void Animation::update_local_ghetto(CBaseEntity* local)
{
	...
	if (tickcount != Interfaces::Globals->tickcount) {
		memcpy(layers, local->GetAnimOverlays(), sizeof(CAnimationLayer) * local->GetNumAnimOverlays());

		local->clientsideanim();
		local->GetAnimState()->m_iLastClientSideAnimationUpdateFramecount = 0;
		
		local->UpdateAnimationState(local->GetAnimState(), G::UserCmd->viewangles);

		if (G::SendPacket) {
			G::proper_obs = local->GetAnimState()->m_flGoalFeetYaw;
		...
		local->GetAnimState()->m_iLastClientSideAnimationUpdateFramecount = Interfaces::Globals->framecount;

	local->SetAbsAngles(Vector(0, G::proper_obs, 0));
	local->GetAnimState()->m_flFeetYawRate = 0.f;

	memcpy(local->GetAnimOverlays(), layers, sizeof(CAnimationLayer) * 13);
	memcpy(local->GetPoseParamaters(), pp, sizeof(float) * 24);
	*/
	static float proper_abs = local->GetAnimState()->m_flGoalFeetYaw;
	static std::array<float, 24> sent_pose_params = local->m_flPoseParameter();

	static CAnimationLayer backup_layers[15];
	static int tick = 0;
	if (tick != Interfaces::Globals->tickcount) // Only update animations each tick, though we are doing this each frame.
	{
		tick = Interfaces::Globals->tickcount;
		std::memcpy(backup_layers, local->GetAnimOverlays(), (sizeof(CAnimationLayer) * local->GetNumAnimOverlays()));
		//local->m_bClientSideAnimation() = true; //just does stuff like set m_bClientSideAnimation and m_iLastAnimUpdateFrameCount
		local->UpdateAnimationState(local->GetAnimState(), local->GetEyeAngles());
		if (G::SendPacket)
		{
			proper_abs = local->GetAnimState()->m_flGoalFeetYaw;
			sent_pose_params = local->m_flPoseParameter();
		}
	}
	//local->m_bClientSideAnimation() = false;
	local->SetAbsAngles(Vector(0, proper_abs, 0));
	local->GetAnimState()->m_flUnknownFraction = 0.f; // Lol.
	std::memcpy(local->GetAnimOverlays(), backup_layers, (sizeof(CAnimationLayer) * local->GetNumAnimOverlays()));
