...
void c_config::save() {
	...
	sprintf(file_path, "C:/vos/cfg/csgo_%s.ini", presets.at(i["_preset"]));
...
void c_config::load_keys() {
	for (int k = 0; k < this->presets.size(); k++)
	{
		...
		this->i[buffer] = GetPrivateProfileIntA("k", buffer, 0, "C:/vos/cfg/csgo_keys.ini");
	}
}
...
void c_config::save_keys()
{
	for (int k = 0; k < presets.size(); k++) {
		...
		WritePrivateProfileStringA("k", buffer, value, "C:/vos/cfg/csgo_keys.ini");
	}
}